...
import { listTemplatePrompt, createNewTemplatePrompt } from '../../api/templates';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { DragDropContext, Droppable, Draggable, DropResult } from 'react-beautiful-dnd';
import { FaGripVertical, FaArrowLeft } from 'react-icons/fa';
import { useLocation, useNavigate } from 'react-router-dom';
import {Prompt, DuplicateTemplateProps} from '../../components/Interfaces'
import './index.css';


const DuplicateTemplate: React.FC = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { template } = location.state as DuplicateTemplateProps;

  const [prompts, setPrompts] = useState<Prompt[]>([]);
  const [tempPrompts, setTempPrompts] = useState<Prompt[]>([]);
  const [templateName, setTemplateName] = useState<string>(`${template.title}_COPY`);
  ...
  return (
    <Container>
      ...
      <h1>Duplicate Report Template</h1>
      ...
      <Form>
        ...
        <Form.Group controlId="customPromptType" className='mt-2'>
          ...
          >
            <option value="ReportText">AI Generated Text</option>
          </Form.Select>
        </Form.Group>
